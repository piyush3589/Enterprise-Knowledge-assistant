...
Failure to follow lockout/tagout procedure may result in serious injury or death.
""",
...
INSPECTION POINTS (12 locations marked with yellow paint):
- IP-01 through IP-06: Straight sections, 2-metre intervals
- IP-07 through IP-10: Bends and elbows (highest corrosion risk)
- IP-11, IP-12: Pipe supports (risk of external corrosion under support)
...
PURPOSE:
Prevent accidental energisation of equipment during maintenance,
protecting personnel from electrical shock, arc flash, and unexpected startup.
...
- Stored mechanical energy: Block any moving parts that could fall or move
...
Any near-miss must be reported within 1 hour to the safety officer.
""",
...
APPLICABLE TO: All pressure vessels and boilers at Plant A and Plant B

STATUTORY REQUIREMENTS:
- All pressure vessels above 25 litres capacity require registration with the Chief Inspector of Factories
...
- Pressure vessels must not operate beyond the certified maximum allowable working pressure (MAWP)

INSPECTION CATEGORIES:
1. EXTERNAL INSPECTION (Annual):
   - Visual examination of shell, heads, nozzles, supports
   - Check insulation condition, corrosion under insulation (CUI)
   - Verify safety relief valves are certified and within test date
   - Inspect pressure gauges for calibration certificate

2. INTERNAL INSPECTION (Every 2 years or as specified):
   ...
   - Check all internal fittings, baffles, tubes
...
- Material certificates for pressure parts
- Weld procedure specifications and welder qualifications
- Inspection reports for all past inspections
- Repair history
...
STEP 4 — Check for leaks
  ...
  → Look for puddles, wet insulation, dripping joints
  → If leak found: Isolate section, repair, pressure test
  → If no leak found: Go to Step 5

STEP 5 — Check isolation valves
  → Verify all isolation valves in coolant circuit are fully open
  → IV-301, IV-302, IV-303, IV-304 — all should be fully open (handwheel fully counterclockwise)
  → If any valve partially closed: Open fully and monitor flow recovery

STEP 6 — Mechanical inspection
  → If none of above resolves: Suspect pump impeller wear or cavitation
  → Arrange mechanical inspection of pump P-03 during next planned shutdown
  → Consider increasing discharge pressure to rule out cavitation
...
- High frequency (> 1000 Hz): Bearing defect (inner race, outer race, or rolling element)
- Broadband noise: Cavitation or flow turbulence
- Subsynchronous (< 1x RPM): Instability, rub, or loose component
...
- Loose components: Check all hold-down bolts, coupling bolts, pipe supports

RETURN TO SERVICE: Vibration must be < 2.5 mm/s RMS after any bearing or alignment work before returning to normal operation.
""",
...
def seed():
    ...
    print(f"Seeded {len(DOCUMENTS)} documents into ChromaDB at {CHROMA_PATH}")
